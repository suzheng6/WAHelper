...
def main() -> int:
    ...
    make_zip(include_userdata=False, zip_name="WAHelper_分发.zip")
    # 可选：本机完整备份 zip（含登录），默认不生成以节省时间；需要时设 WA_ZIP_WITH_USERDATA=1
    if os.environ.get("WA_ZIP_WITH_USERDATA", "").strip() in ("1", "true", "yes"):
        make_zip(include_userdata=True, zip_name="WAHelper_本机完整.zip")

    print(f"\n完成。")
    print(f"  本机运行目录（含你的登录数据）：{OUT}")
    print(f"  对外分发包（无登录/配置）：{DIST / 'WAHelper_分发.zip'}")
